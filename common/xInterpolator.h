 ...
 ********************************************************************
 *  Base class xInterpolator provides a common interface for        *
...
public:
    ...
    { 
        ...
        if (reader.codProperties.table.Handler != HANDLER_xInterpolator)  return 0; // Found the ID but has a wrong handler
...
protected:
    ...
    uint16_t GetRef(const char* filename, uint16_t ID, REFT* data, uint32_t dataBytes) // __attribute__((optimize("Og"))) 
    {
        ...
        if (reader.codProperties.table.Handler != HANDLER_xInterpolator) { reader.Close(); return 0; } // Check table has the correct handler
...
public:
    ...
    bool Init(REFT* data, uint16_t size)  {
        ...
        // STDOUT.print("[xInterpolator.Init] Initialized with "); STDOUT.print(refSize); STDOUT.print(" references:");
...
template<class REFT, class WORKT>
class LUT  : public xInterpolator<REFT, WORKT> {
    // Bring dependent names in scope (lost when binding templates against inheritance):
    using xInterpolator<REFT, WORKT>::refSize;          
    using xInterpolator<REFT, WORKT>::refData;          
    using xInterpolator<REFT, WORKT>::precalcData;      // | slope[0]| slope[1] | ... | slope[N-2] |
    using xInterpolator<REFT, WORKT>::state;
// public:
private:
...
template<class REFT, class WORKT>
class TF  : public xInterpolator<REFT, WORKT> {
    // Bring dependent names in scope (lost when binding templates against inheritance):
    using xInterpolator<REFT, WORKT>::refSize;          
    using xInterpolator<REFT, WORKT>::refData;          
    using xInterpolator<REFT, WORKT>::precalcData;      // | slope[0]| slope[1] | ... | slope[N-2] |
...
void XInt_test(void) {  
    ...
    // scalableTF<int16_t, int32_t> testTF;  // scroll transfer function. 
    // int16_t TFdata[4] = {     10,    110,       // X = scrollTime (decreases in time)
    //                         500,    100    }; // Y = scrollTickPeriod [ms], specified as TPS(ticks_per_second)



    // #define REFtype float
    // #define WORKtype float

    // volatile WORKtype result[51];
    // REFtype data[6] = {10, 20, 20, 19, 50, 0};
    // WORKtype speedupRAM[5];
    // static LUT<REFtype, WORKtype> testLUT(-11, 39);
    // testLUT.Init(data,6);
    // testLUT.Start();
    // // if (!testLUT.SpeedUp(speedupRAM, sizeof(speedupRAM))) STDOUT.println("Speedup failed");
    // STDOUT.println("NEW TEST ----------------------------");
    // noInterrupts();
    // uint32_t starttime = micros();
    // for (int16_t i=-11; i<=39; i++) {    
    //     // STDOUT.print("test[");  STDOUT.print(i); STDOUT.print("] = ");
    //     // STDOUT.println(testLUT.Get(i));
    //     result[i+11] = testLUT.Get(i);
    // }
    // float time = ((float)(micros()-starttime)) / 50;
    // interrupts();
    // for (uint8_t i=0; i<=50; i++) {
    //     STDOUT.print(result[i]); STDOUT.print("; "); }
    // STDOUT.println("");
    // STDOUT.print("No-speedup duration:"); STDOUT.print(time); STDOUT.println("microseconds.");
    
    // if (!testLUT.SpeedUp(speedupRAM, sizeof(speedupRAM))) STDOUT.println("Speedup failed");
    // noInterrupts();
    // starttime = micros();
    // for (int16_t i=39; i>=-11; i--) {    
    //     // STDOUT.print("test[");  STDOUT.print(i); STDOUT.print("] = ");
    //     // STDOUT.println(testLUT.Get(i));
    //     result[i+11] = testLUT.Get(i);
    // }
    // time = ((float)(micros()-starttime)) / 50;
    // interrupts();
    // for (uint8_t i=0; i<=50; i++) {
    //     STDOUT.print(result[i]); STDOUT.print("; "); }
    // STDOUT.println("");
    // STDOUT.print("Speedup duration:"); STDOUT.print(time); STDOUT.println("microseconds.");

    // #define REFtype uint8_t
    // #define WORKtype float

    // volatile WORKtype result[51];
    // REFtype data[12] = {3,  13, 15, 20, 40, 53, 
    //                     10, 20, 20, 19, 50, 0};
    // WORKtype speedupRAM[5];
    // static TF<REFtype, WORKtype> testTF;
    // testTF.Init(data,12);
    // testTF.Start();
    // // if (!testLUT.SpeedUp(speedupRAM, sizeof(speedupRAM))) STDOUT.println("Speedup failed");
    // STDOUT.println("NEW TEST ----------------------------");
    // noInterrupts();
    // uint32_t starttime = micros();
    // for (int16_t i=3; i<=53; i++) {    
    //     result[i-3] = testTF.Get(i);
    // }
    // float time = ((float)(micros()-starttime)) / 50;
    // interrupts();
    // for (uint8_t i=0; i<=50; i++) {
    ...
    // if (!testTF.SpeedUp(speedupRAM, sizeof(speedupRAM))) STDOUT.println("Speedup failed");
    // noInterrupts();
    // starttime = micros();
    // for (int16_t i=3; i<=53; i++) {    
    //     result[i-3] = testTF.Get(i);
    ...
    // static TF<REFtype, WORKtype> testTF;
    // uint16_t num;
    // STDOUT.println("LUT TEST ----------------------------");
    // // num = testTF.FindRef("testLUT.dat", 99);
    // // STDOUT.print("LUT 99 has "); STDOUT.print(num); STDOUT.println(" bytes.");
   
    // num = testTF.Init("testLUT.dat", 99, data, 12);
    // STDOUT.print("LUT 99 init state: "); STDOUT.print(num); STDOUT.println(".");
    // // num = testTF.Init("testLUT.dat", 99, data, 12);
    // // STDOUT.print("LUT 99 init state: "); STDOUT.print(num); STDOUT.println(".");
    ...
    // STDOUT.println("END OF LUT TEST ---------------------");

    // #define REFtype int16_t
    // #define WORKtype uint16_t
    // static TF<REFtype, WORKtype> testTF;
    // REFtype data[14];

    // testTF.Init("LEDs/XP2E-Red.dat", 12, data, sizeof(data));
}
