...
// Test purpose define TODO remove after release 
// #define COD_TEST_ACTIVE
...
public: 
    ...
    *   @retval :
    */
    xCodInterpreter(File * pFile, CodEntry* codProperties, uint8_t *entryType) 
    {
        this->pFile = pFile;
...
class xCodInterpreter_V2 : public xCodInterpreter {
public:
...
public:
    ...
    *   @retval :
    * */
   xCodInterpreter_V2(File * pFile, CodEntry* codProperties, uint8_t *entryType) : xCodInterpreter(pFile, codProperties, entryType)
   {
...
class xCodInterpreter_V1 : public xCodInterpreter {
...
public: 
    ...
    *
    * */
    xCodInterpreter_V1(File * pFile, CodEntry* codProperties, uint8_t *entryType) : xCodInterpreter(pFile, codProperties, entryType)
   {
...
class xCodInterpreter_V3 : public xCodInterpreter_V2 {
    private:
    ...
    *   @retval :
    * */
   xCodInterpreter_V3(File * pFile, CodEntry* codProperties, uint8_t *entryType) : xCodInterpreter_V2(pFile, codProperties, entryType)
   {
...
public:
    CodEntry codProperties; // ID, handler and size
    uint8_t entryType;      // COD_ENTYPE_TABLE=1, COD_ENTYPE_STRUCT=2
    xCodInterpreter *codInterpreter;
...
public:
    ...
    *  @retval  : void   
    */
    xCodReader() //__attribute__((optimize("O0")))
    {
    ...
    */
    ...
    {   
        ...
        //xCodReader();   // reset ptivate memebers to default 
    }
    ...
    */
    bool ReadHeader()  //__attribute__((optimize("Og")))
    {
        ...
        if(!strcmp((char*)&headerName[0], "cod_v01")) {
            codInterpreter = new xCodInterpreter_V1(&file, &codProperties, &entryType);
        ...
        {   
            codInterpreter = new xCodInterpreter_V2(&file, &codProperties, &entryType);
            if(!codInterpreter) return false;   // allocation failed
        ...
        {
            codInterpreter = new xCodInterpreter_V3(&file, &codProperties, &entryType);
            if(!codInterpreter) return false;   // allocation failed
            uint32_t readCRC, calcCRC;
            calcCRC = codInterpreter->GetFileCrC(&readCRC);
            if(calcCRC != readCRC)
...
#ifdef COD_TEST_ACTIVE

// ================================ TEST START =================================================
static int16_t databuffer1_int16[100];
static uint16_t databuffer2_uint16[100];
static uint8_t databuffer3_uint8[40];
static uint8_t databuffer5_uint8[40];
static float  databuffer4_float[20];
static float  databuffer6_float[20];

static uint8_t file_bfr[228];
...
testStruct testStructVar1;


void xCodReader_TestFunction()
{
    bool result;
    int8_t res;
    xCodReader codReaderObj;
    uint32_t readCRC, calcCRC;
    uint16_t idsList[10] = {7, 8, 55, 69, 98, 99, 20, 100, 10, 9};

    // result = codReaderObj.Open("testV1.cod");

    result = codReaderObj.Open("testV1Crc.cod");            // test1V2Crc.cod       // testV2Crc
...
// #endif
    if(result) 
    {
        ...
        {
            res = codReaderObj.FindEntry(idsList[i]);
            if(res > 0)
            {   
                if(res == COD_ENTYPE_TABLE) 
                {
                    ...
                    switch(codReaderObj.codProperties.table.DataType)
                    {
                        case 1:
                            result = codReaderObj.ReadEntry(codReaderObj.codProperties.table.ID, (void*)&databuffer3_uint8, 40);
                            if(result){
                                STDOUT.println("data Read");
                                for(uint16_t i=0; i< codReaderObj.codInterpreter->entryDataSize; i++){
                                    STDOUT.print(databuffer3_uint8[i]);
                                    STDOUT.print(" ");
                                }
                                STDOUT.println(" ");
                            }
                            else 
                                STDOUT.println("failed to read data ");
                        break;

                        case 2:
                            result = codReaderObj.ReadEntry(codReaderObj.codProperties.table.ID, (void*)&databuffer1_int16, 100);
                            if(result){
                                STDOUT.println("data Read");
                                for(uint16_t i=0; i< codReaderObj.codInterpreter->entryDataSize/2; i++)
                                {
                                    STDOUT.print(databuffer1_int16[i]);
                                    STDOUT.print(" ");
                                }
                                STDOUT.println(" ");
                            }
                        ...
                        case 3:
                            result = codReaderObj.ReadEntry(codReaderObj.codProperties.table.ID, (void*)&databuffer4_float, 20 * 4);
                            if(result){
                                STDOUT.println("data Read");
                                for(uint16_t i=0; i< codReaderObj.codInterpreter->entryDataSize/4; i++)
                                {
                                    STDOUT.print(databuffer4_float[i]);
                                    STDOUT.print(" ");
                                }
                                STDOUT.println(" ");
                            }
                    ...
                    result = codReaderObj.ReadEntry(codReaderObj.codProperties.structure.ID, (void*)&testStructVar, codReaderObj.codProperties.structure.Size);

                    if(result) {
                        STDOUT.println("data Read");
                        STDOUT.print(testStructVar.field1);
                        STDOUT.print(" ");
                        //float nrToPrint = (float)((testStructVar.field2[0] << 24) | (testStructVar.field2[1] << 16) | (testStructVar.field2[2] << 8) | testStructVar.field2[3]); 
                        // float nrToPrint =*(float *)&(testStructVar.field2);
                        STDOUT.print(testStructVar.field2);
                        STDOUT.print(" ");
                        for(uint8_t i=0; i < 13; i++)
                            STDOUT.print(testStructVar.field3[i]);
                        STDOUT.println(" ");
                    }
                    else 
                        STDOUT.println("failed to read data ");
                    ...
                    STDOUT.print("Entry type unknown on ID "); STDOUT.println(idsList[i]);
                }

            } 
            else if(res == 0 )
            {
                STDOUT.print("Entry not found ID = "); STDOUT.println(idsList[i]);
            } else {
                STDOUT.print("Error on ID = "); STDOUT.println(idsList[i]);
            }
    ...
    codReaderObj.Close();
    uint16_t idsList2[10] = {22, 21, 11, 999, 14, 1, 20, 100, 10, 9};

    STDOUT.println("ANALED START !!");
    result = codReaderObj.Open("anaLed.cod");
    if(result) 
    {
        for(uint8_t i=0; i < 10; i++ )
        {
            res = codReaderObj.FindEntry(idsList2[i]);
            if(res > 0)
            {   
                if(res == COD_ENTYPE_TABLE) 
                {
                    ...
                    switch(codReaderObj.codProperties.table.DataType)
                    {
                        ...
                        case xCod_Uint16:
                            result = codReaderObj.ReadEntry(codReaderObj.codProperties.table.ID, (void*)&databuffer2_uint16, 100);
                            if(result){
                                STDOUT.println("data Read");
                                for(uint16_t i=0; i< codReaderObj.codInterpreter->entryDataSize/2; i++)
                                {
                                    STDOUT.print(databuffer2_uint16[i]);
                                    STDOUT.print(" ");
                                }
                                STDOUT.println(" ");
                            }
                    ...
                    result = codReaderObj.ReadEntry(codReaderObj.codProperties.structure.ID, (void*)&databuffer5_uint8, codReaderObj.codProperties.structure.Size);
                    ...
                    STDOUT.print("==Entry TYPE=="); STDOUT.println(res);
                    STDOUT.print("Entry type unknown on ID "); STDOUT.println(idsList2[i]);
                }

            } 
            else if(res == 0 )
            {
                STDOUT.print("Entry not found ID = "); STDOUT.println(idsList2[i]);
            } else {
                STDOUT.print("Error on ID = "); STDOUT.println(idsList2[i]);
            }
