...
public:
    xMenu (TYPE initialTick = 0) : Looper(1000*MENU_PERIOD) { 
        tick = initialTick; 
        stableTick = tick;
        // rolling = true;
        rolling = false;
    ...
    const char* name() override { return "xMenu"; } 
...
bool SetActions(xMenuActions* actions_, TYPE initialValue)  {     
    ...
    if (!player2) 
        if (player1) STDOUT.println("xMenu not fully functional - WAV player 2 not available"); // we'll have to manage without the background player
        else  { 
            STDOUT.println("Cannot start xMenu - WAV player 1 not available");
...
// NAV HELPERS:
    ...
    void StartScroll(uint32_t strength) {
        ...
        // STDOUT.print("[xMenu.StartScroll] initial, un-processed scrollIndex = "); STDOUT.println(scrollIndex);
        ...
        // STDOUT.print("[xMenu.StartScroll]  After processing: scrollIndex = "); STDOUT.print(scrollIndex); STDOUT.print(", scrollTickPeriod = "); STDOUT.println(scrollTickPeriod);
    ...
    // #endif // X_MENUTEST
        ...
        if (buttonState && !skipping) 
            if (abs(fusor.theta()) >= MENU_SKIPTH) {
                skipping = true;
               // STDOUT.println("[xMenu.Event] start skipping"); 
        ...
        switch (eventID) {
            ...
            case EVENT_RELEASED:    // stop skipping
                ...
                if (skipping) {
                    ...
                    //STDOUT.println("[xMenu.Event] stop skipping"); 
                }
            break;

            case EVENT_CLICK_SHORT: // confirm
                // STDOUT.println("[xMenu.Event] CONFIRMED"); 
                // actions->currentValue = tick;
                if(player2) {
                    player2->Stop();
                    player2->CloseFiles();
            ...
            case EVENT_HELD_LONG:   // cancel
                if (!skipping) {
                    ...
                    // STDOUT.println("[xMenu.Event] CANCELED"); 
                    // actions->currentValue = tick;
                    if(player2) {
                        player2->Stop();
                        player2->CloseFiles();
...
// class testMenu_t : public xMenu<uint16_t>::xMenuActions {
