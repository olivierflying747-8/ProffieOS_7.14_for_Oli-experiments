// attempt 032 (working version for saber-blaster-detonator-jetpack, all with sound-effects on transition between props.
...
#include "../props/blaster_BC_buttons.h"        //use BlasterBC            | Compiling with multi_prop.h (after modifications) must include "bullet count"
...
public:
  ...
  uint32_t reverse_map_button(uint32_t b) {
    ...
    void SB_Effect(EffectType effect, float location) override {    // something is wrong here
                                                                    // Your SB_Effect() only checks the mode, not what
                                                                    // type of request was requested, which means that
                                                                    // every effect will play the mode change sound.
        switch (currentMode) {
            case PropMode::SABER:
